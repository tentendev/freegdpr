...
<a href="https://yourdomain.com" target="_blank" style="display:inline-flex;align-items:center;gap:6px;padding:6px 12px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:20px;text-decoration:none;color:#475569;font-family:sans-serif;font-size:12px;font-weight:500;">
  ...
  const handleShare = (platform: 'twitter' | 'linkedin') => {
    const text = encodeURIComponent(t.result.shareText);
    const url = encodeURIComponent("https://freegdpr.com");
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-lg overflow-hidden border border-slate-100 flex flex-col">
          <div className="bg-slate-50 border-b border-slate-200 p-4 flex justify-between items-center flex-wrap gap-2">
            <h3 className="font-semibold text-slate-700">{t.result.documentPreview}</h3>
             <div className="flex gap-2">
              ...
              >
                ...
                {viewMode === 'preview' ? t.result.showHtml : t.result.showPreview}
              </button>
              <button
                onClick={handleCopy}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-md border border-slate-100">
            <h3 className="font-bold text-slate-900 mb-4">{t.result.whatsNext.title}</h3>
            <ul className="space-y-3">
              {t.result.whatsNext.steps.map((step, index) => (
                <li key={index} className="flex gap-3 text-sm text-slate-600">
                  <div className="w-6 h-6 rounded-full bg-green-100 text-green-600 flex items-center justify-center shrink-0 text-xs font-bold">{index + 1}</div>
                  {step}
                </li>
              ))}
            </ul>
          ...
          <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
            <div className="flex items-center gap-2 mb-3">
               ...
               <h3 className="font-bold text-slate-900 text-sm">{t.result.badge.title}</h3>
            </div>
            <p className="text-xs text-slate-500 mb-4">
              {t.result.badge.description}
            </p>
